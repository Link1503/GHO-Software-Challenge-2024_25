...
public class Logic implements IGameHandler {
	...
	public String[] ZugZuortnung(Field[] FelderListe, int[] Werte){
		...
		for (int i = 1; i < FelderListe.length; i++) {
   ...
   public float Floatwerte_Anpassung (int[] VorherigeWerte, int [] AktuelleWerte, float Floatwert, String Zug){
	   ...
	   float Postion = 0.0f;
	   float Karrotten = 0.01f;
	   float SWAP_CARROT = 0.1f;
	   float EAT_SALAD = 0.1f;
	  switch(Zug){ 
	  ...
	  case "Ziel Funktion","Vor Ziel Funktion", "Salat Funktion", "Markt Funktion", "Hasen Funktion", "Erste Position Funktion", "Zweite Position Funktion", "Karrotten Funktion":{
	    for(int i = 0; i < Gegangende_Dist; i++ ) {
	    	Postion = Postion + 0.01f;
	    }
	    break;
	  }
	  case "Karrotten abgeben Funktion":{
	  ...
	  case "Igel Funktion":{
	    Karrotten = Karrotten * KarrottenZähler;
	    break;
	   }
	 
	}
	  Floatwert = Floatwert + Postion + Karrotten + SWAP_CARROT + EAT_SALAD;
	   return Floatwert; 
   }
   ...
   public float[][] Floatwerte( String[][] Züge ) {
	   ...
	   for (int i = 0; i < Züge.length; i++){
	       for (int j = 0; j < Züge[i].length ; j++){
	    	   Zug = Züge [i][j];
	    	   switch(Zug)  {
	    	   case "Salatfressen Funktion " : Floatwert = 0.5f;
	    	   case "Ziel Funktion": Floatwert = 1.0f;
	    	   case "Karrotten abgeben Funktion": Floatwert = 0.1f;
	    	   case "Vor Ziel Funktion": Floatwert = 0.5f;
	    	   case "Salat Funktion": Floatwert = 0.5f;
	    	   case "Markt Funktion": Floatwert = 0.3f;
	    	   case "Hasen Funktion": Floatwert = 0.3f;
   ...
   public float Endwert(float[][] Float, String[][] Züge ) {
    ...
    int[] Felderint =  IndexArrFürFeldtyp(gameState.getBoard(), Field.CARROTS);
